...
# Torch tensor transform from numpy 
...
train_val_data, test_data, train_val_labels, test_labels = train_test_split(X, Y, test_size = 0.3, stratify = Y, random_state = 42)
...
class ECGClassifier(nn.Module):
    def __init__(self):
        ...
        self.ECG_seq = nn.Sequential(nn.Conv1d(1, 32, kernel_size = 2, stride = 2, padding = 'valid'),
                                     nn.ReLU(), 
                                     nn.BatchNorm1d(32),

                                     nn.Conv1d(32, 64, kernel_size = 2, stride = 2, padding = 'valid'),
                                     nn.ReLU(), 
                                     nn.BatchNorm1d(64),
        ...
        self.lstm1 = nn.LSTM(input_size = 498, hidden_size = 64, num_layers = 1, batch_first = True)
...
#optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=0.05, betas=(0.9,0.999))
...
for fold, (train_index, val_index) in enumerate(skf.split(dataset.data, dataset.labels)):
    ...
    print('The class distribution after the oversampling is:', counter)
    
    # Transforming back to Torch Tensors
    X_train = torch.tensor(X_train)
    Y_train = torch.tensor(Y_train)

    # Creating the New Training Data Set
    ...
    for epoch in range(n_epochs):
        ...
        for i, (data, labels) in enumerate(train_loader):
            ...
            for i in range(len(predicted_train)):
              if labels[i] > 0.99:
                true_labels_AF += 1
              if predicted_train[i] > 0.55 and labels[i] > 0.5:
...
fpr, tpr, _ = metrics.roc_curve(y_true_AUC,  y_predi_AUC)
auc_plt = metrics.roc_auc_score(y_true_AUC, y_predi_AUC)

plt.plot(fpr,tpr,label="AUC="+str(auc_plt))
